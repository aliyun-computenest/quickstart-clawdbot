...
cmd_get_url() {
  ...
  echo "换浏览器、换电脑或用无痕窗口打开都相当于一台新设备，需要重新取一条链接。"
  ...
  # 有效期与一次性由上面 get-login-url 的输出交代，这里只补它没说的：尽快打开、如何再取。
  echo "  建议取到后立刻打开。过期、已用过或要换设备时，执行运维操作「获取控制台"
  echo "  登录链接」重新获取即可。"
}
...
cmd_get_login_url() {
  ...
  # 换设备需重新取链接这点必须写在这里：用户执行本运维操作时看不到部署 Outputs 的说明。
  echo "提示: 用浏览器打开上面的链接完成首次登录后，该浏览器即可长期使用「控制台地址 + 访问令牌」访问；换浏览器、换电脑或用无痕窗口打开都相当于一台新设备，需要重新取链接；链接过期、已被用过或要换设备时，重新执行本运维操作即可再次获取。"
}
