...
5. 在服务实例详情页的部署输出中获取控制台地址与访问令牌。

## 部署完成后的必读事项

### 首次打开控制台

在浏览器打开部署输出里的控制台地址，填入访问令牌登录。自 openclaw 2026.8.1 起，即使通过明文 HTTP 访问，控制台也要求每台新设备完成配对，所以首个浏览器会停在「需要设备配对」页面，并显示一个配对请求 ID。

此时用部署时设置的 root 密码登录这台 ECS，执行以下命令批准：

```bash
docker exec openclaw openclaw devices approve <配对请求ID>
```

命令执行后浏览器即可进入主界面。**openclaw 命令只存在于容器内，必须带 `docker exec openclaw` 前缀**，直接执行 `openclaw` 会报 `command not found`。若页面上没有显示请求 ID，可先用下面的命令从 Pending 一栏查到它：

```bash
docker exec openclaw openclaw devices list
```

该浏览器批准过一次后，之后凭「控制台地址 + 访问令牌」即可长期访问。**更换浏览器、更换电脑或使用无痕窗口访问时，都相当于一台新设备，需要再批准一次**。
...
| 运维操作 | 用途 |
|----------|------|
| 获取控制台登录链接 | 重新输出控制台地址与访问令牌 |
| 配置频道 | 接入钉钉、企业微信、QQ、飞书 |
| 配置百炼CodingPlan | 补齐或更换百炼 API Key |
| 变更Skill配置 | 调整随实例安装的 Skills |
| 重置服务 | 清空全部配置、会话、工作区与已安装 Skills；访问令牌保留，但设备配对记录会被清除，需重新批准一次设备 |
