...
5. 在服务实例详情页的部署输出中获取控制台地址、访问令牌与一次性登录链接。

## 部署完成后的必读事项

### 首次打开控制台

自 openclaw 2026.8.1 起，即使通过明文 HTTP 访问，控制台也要求首个浏览器完成设备配对，仅凭访问令牌无法放行。因此请使用部署输出中的一次性登录链接完成首次登录，该浏览器随后即可长期使用「控制台地址 + 访问令牌」进入。

一次性链接十分钟内有效且仅能使用一次，建议获取后立即打开。**更换浏览器、更换电脑或使用无痕窗口访问时，都相当于一台新设备，需要重新获取链接**。链接过期、已被使用或需要为新设备取用时，均可执行运维操作「获取控制台登录链接」重新生成。
...
请注意，使用一次性链接登录控制台得到的是设备操作权限（operator），它本身并不是命令负责人，而是有权去指定命令负责人。指定方式二选一：
...
| 运维操作 | 用途 |
|----------|------|
| 获取控制台登录链接 | 为新设备生成一次性登录链接 |
| 配置频道 | 接入钉钉、企业微信、QQ、飞书 |
| 配置百炼CodingPlan | 补齐或更换百炼 API Key |
| 变更Skill配置 | 调整随实例安装的 Skills |
| 重置服务 | 清空全部配置、会话、工作区与已安装 Skills；访问令牌保留，但设备配对记录会被清除，需重新取一次性链接登录 |
